...
rejectCalcs = 45 # reject anything that is below this BPM threshold
waterToCount = 10 # number of heart beats to sample before calculating an average BPM
...
def parse_command_line_args():
    ...
    parser.add_argument(
            ...
            default=gpioIN,
	    type=int,
            help=('GPIO input pin for the water sensor'))
...
def calc(startTime, endTime):
    ...
    calcs = (60/sampleSeconds)*(waterToCount)
    return calcs
...
def main():
    ...
    instantCalcs = 0 # BPM calculated from the time between two heartbeats
            ...
            if inputReceived == 1:
                if previousInput == 0: # the heart beat signal went from low to high
                    ...
                    if totalSampleCounter == 1: # the very first beat since the program started running
                        print "Receiving water consumption signals"
                    ...
                    if sampleCounter == -1: # the first beat received since the counter was reset
                        ...
                        instantCalcs = 60/(thisPulseTime - lastPulseTime)
                        ...
                        if instantCalcs < rejectCalcs: # this heart rate is likely due to a bad connection
                            sampleCounter = -1 # reset counter so that the bad data isn't included in a BPM average

                    if sampleCounter == waterToCount: # time to calculate the average BPM
